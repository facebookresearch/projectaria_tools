...
VRS Health Check is a comprehensive tool for analyzing and validating VRS recording files collected by Aria devices.
...
:::caution No ARM64 build
Unlike `projectaria-client-sdk` and `projectaria-tools`, VRS Health Check publishes **no `aarch64` wheel**. This is what keeps [MPS](/ark/mps/start) and [Aria Studio](/ark/aria_studio/start) — both of which depend on it — off ARM64 Linux.
:::

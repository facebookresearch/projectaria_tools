...
- [Example code exported](/ark/client-sdk/python-sdk/python-interface#export-example-code)
- Port 6768 available and not blocked by firewall

:::caution One-Time Setup: Device Authentication Required
Before running this example, you **must** authenticate your device by running `aria_gen2 auth pair` (see [Device Authentication](/ark/client-sdk/authentication)). This only needs to be done **once** per device-PC combination, but **all device operations (recording, streaming, etc.) will fail without it**.
:::
...
This section demonstrates how to stream data to a remote server running at a specific IP address and port. This is useful when you want to run your streaming receiver on a different machine than the one connected to the Aria device.

### Overview

Remote streaming allows you to:
- **Run the receiver on a dedicated server**: Process streaming data on a more powerful machine
- **Stream across the network**: Send data to a receiver running on a remote IP address
- **Secure communication**: Use TLS certificates for encrypted streaming

### Workflow Overview

To set up remote streaming, follow these steps in order:

1. **Install certificates** on the device (outputs a cert name)
2. **Copy certificates** to the remote server
3. **Start the streaming server** on the remote machine
4. **Start streaming** on the device with the cert name

:::warning Important
Always start streaming with the correct certificate name or install certificates **before** starting the streaming server. Starting streaming without specifying the cert name will default to installing new certificates, which would invalidate any old certificates already copied to the server. This is the same rule that governs the local start order — see [Start Order and Streaming Certificates](/ark/client-sdk/streaming#start-order-and-streaming-certificates).
:::

---

### Step 1: Install Streaming Certificates

On the machine connected to the Aria Gen2 device via USB, install the streaming certificates:
...
This command:
- Generates new streaming certificates
- Installs the publisher certificate on the Aria Gen2 device
- Saves the subscriber certificates locally to `~/.aria/streaming-certs/persistent/`
- Outputs the **cert name** that you'll use when starting streaming
...
**Note the cert name** (e.g., `cert_1769712774692532015`) - you'll need it in Step 4.

#### Certificate Files

After installation, the following files are created in `~/.aria/streaming-certs/persistent/`:

| File | Description |
|------|-------------|
| `root_ca.pem` | Root CA certificate that verifies the device's publisher certificate |
| `subscriber.pem` | Subscriber certificate for the streaming server |
| `subscriber-key.pem` | Subscriber private key for the streaming server |
| `publisher-cert-name` | Contains the certificate name |

---

### Step 2: Copy Certificates to Remote Server

Copy the certificate directory to your remote server:
...
On the remote server, start the streaming server with the copied certificates.
...
def setup_remote_streaming_server(certs_dir):
    """Configure streaming receiver for remote connections with TLS certificates."""

    # Certificate paths from copied directory
    root_ca_path = os.path.join(certs_dir, "root_ca.pem")
    subscriber_cert_path = os.path.join(certs_dir, "subscriber.pem")
    subscriber_key_path = os.path.join(certs_dir, "subscriber-key.pem")

    # Configure server with remote IP and port
    config = sdk_gen2.HttpServerConfig()
    config.address = "0.0.0.0"  # Listen on all interfaces for remote connections
    config.port = 6768           # Default streaming port

    # Configure TLS with subscriber certificates
    config.ca_root = root_ca_path
    config.certificate.cert = subscriber_cert_path
    config.certificate.key = subscriber_key_path

    # Create and configure the receiver
    stream_receiver = receiver.StreamReceiver(enable_image_decoding=True)
    stream_receiver.set_server_config(config)

    # Register callbacks (same as local streaming)
    stream_receiver.register_slam_callback(image_callback)
    stream_receiver.register_rgb_callback(image_callback)
    stream_receiver.register_audio_callback(audio_callback)
    stream_receiver.register_eye_gaze_callback(eyegaze_callback)
...
### Step 4: Start Streaming on Device

On the machine connected to the Aria device, start streaming with the cert name from Step 1:

```bash
# Start streaming to the remote server
aria_gen2 streaming start \
    --url https://<remote-server-ip>:<remote-server-port> \
    --streaming-cert-name <cert-name-from-step-1>
```

**Options:**
- `--url`: The URL of the remote streaming server (e.g., `https://192.168.1.100:6768`)
- `--streaming-cert-name`: The cert name output from Step 1 (e.g., `cert_1767647798330102000`)

---

### Network Configuration

Ensure the following for remote streaming:

1. **Firewall**: Open port 6768 (or your custom port) on the remote server
2. **Network connectivity**: The Aria Gen2 device must be able to reach the remote server IP
3. **Certificate matching**: The publisher certificate on the device must match the subscriber certificates on the server (issued from the same Aria CA)

### Troubleshooting Remote Streaming
...
**Certificate errors:**
- Ensure all certificate files exist in the specified directory
- Verify you installed certs (`aria_gen2 streaming install-certs`) **before** copying them to the server
- If you ran `streaming start` after copying certs, new certs will been installed - reinstall and recopy
- Check that the cert name used in `--streaming-cert-name` matches the one from `install-certs` output

**No data received:**
- Confirm the device started streaming successfully
- Check network connectivity between device and server
- Verify the streaming URL is correct

---

## WebRTC Streaming

The SDK exposes **WebRTC** through the same receiver interface as HTTP. To switch transport you keep every callback and swap only the receiver class (`StreamReceiver` → `WebRtcStreamReceiver`) and its config (`HttpServerConfig` → `WebRtcConfig`). The typed sensor objects your callbacks receive are identical, and H.265 frames are decoded in Python by the same `ImageDecoder`.

### Receiver Setup

This mirrors [`setup_streaming_receiver`](#step-5-set-up-the-streaming-receiver) above — note that only the config and receiver class differ; the callback registrations are unchanged:
...
def setup_webrtc_receiver():
    # Swap ONLY the config: WebRtcConfig instead of HttpServerConfig
    config = sdk_gen2.WebRtcConfig()
    config.listen_port = 9000   # same-network direct peer-to-peer
    config.enable_audio = True

    # Swap ONLY the receiver class: WebRtcStreamReceiver instead of StreamReceiver
    stream_receiver = receiver.WebRtcStreamReceiver()
    stream_receiver.set_server_config(config)

    # The callbacks are IDENTICAL to the HTTP example above
    stream_receiver.register_slam_callback(image_callback)
    stream_receiver.register_rgb_callback(image_callback)
    stream_receiver.register_audio_callback(audio_callback)
    stream_receiver.register_eye_gaze_callback(eyegaze_callback)
    ...
    # Same lifecycle as HTTP: start_server / stop_server / is_connected
    stream_receiver.start_server()
    return stream_receiver
```
...
To reach a device on a different network, point the signaling fields at a signaling server instead of setting `listen_port`. Everything else in `setup_webrtc_receiver` — the receiver class, the callback registrations, the lifecycle — is unchanged:

```python
def webrtc_config_cross_network():
    config = sdk_gen2.WebRtcConfig()
    config.signaling_host = "example.com"
    config.signaling_port = 8443
    config.room_id = "my-room"
    config.room_password = "..."
    config.auth_token = "..."

    # Optional, for NAT traversal:
    config.stun = "stun:example.com:3478"
    config.turn = "turn:example.com:3478"
    config.turn_username = "aria"
...
The signaling server ships as a separate PyPI wheel that you run on a host both the glasses and the receiver can reach — see [WebRTC Streaming](/ark/client-sdk/webrtc). Each field maps to an `aria_streaming_viewer` flag; the pairing is listed in [WebRtcConfig Fields](/ark/client-sdk/python-sdk/python-interface#webrtcconfig-fields).

### WebRTC-only Receiver Methods

Beyond the shared lifecycle (`set_server_config`, `start_server`, `stop_server`, `is_connected`), `WebRtcStreamReceiver` adds methods for the bidirectional WebRTC connection:

```python
# Control the receiver's outbound microphone and incoming audio playback
stream_receiver.set_mic_muted(True)
stream_receiver.set_volume(0.8)

# Read the current audio waveform (e.g. to drive a level meter)
samples, peak = stream_receiver.get_waveform()

# Push your own PCM (a clip, TTS, a synthesized tone) to the device over the
# audio uplink — any sample rate / channel count, float samples in [-1, 1].
stream_receiver.push_audio(pcm_samples, sample_rate=16000, channels=1)
```

:::tip `push_audio` vs the microphone
`push_audio` sends caller-supplied PCM on the host→device uplink from your data instead of the mic hardware — use it for talk-back clips, TTS, or synthesized audio. While pushed audio is active it **replaces** the host microphone on the uplink, so it plays even with the mic muted.
:::

For a complete, runnable script — it starts a WebRTC session, mutes the mic, and plays a generated tone (or a WAV file) to the glasses' speaker — see the **`device_push_audio.py`** sample:

```bash
# Export the samples (once), then run it. This host listens; the device dials it.
...
# Play a 16-bit PCM WAV instead of the default tone:
python ~/Downloads/projectaria_client_sdk_samples_gen2/device_push_audio.py \
...
The callbacks are the integration point, and nothing says they have to print or visualize: the
same handlers can forward what arrives to a model. Prefer WebRTC for this — its lower latency
and the audio path back to the wearer both help when a model is in the loop.

* **Images to a vision-language model.** `register_rgb_callback` (or the SLAM and eye-tracking
  ones) hands you a numpy array per frame, ready for a VLM — hosted API or a local model — for
  scene description, visual Q&A, or grounding what the wearer is looking at.
* **Audio to a realtime model.** `register_audio_callback` delivers PCM as it is captured,
  which is the shape a streaming speech or omni model wants on its input.
* **The answer back to the glasses.** Synthesized speech goes out over the uplink with
  [`push_audio`](#webrtc-only-receiver-methods), so the loop closes inside the same session
  without any second channel.

Eye gaze, hand pose and VIO arrive on the data channel alongside all of this, so a prompt can
carry where the wearer was looking or pointing at the moment the frame was captured.

:::caution Run the inference off the callback thread
Callbacks execute on the receive path. A network round-trip to a model API, or a local forward
pass, blocks delivery for as long as it takes and shows up as dropped frames. Hand the data to
a bounded queue and let a worker consume it — dropping frames when inference falls behind is
what keeps the stream live.
:::
...
Start from the `low_latency_streaming` profile here — it is sized for exactly this kind of
interactive loop, see [Choosing a profile](/ark/client-sdk/webrtc#choosing-a-profile).

### Visualizing a WebRTC Stream

The quickest way to see a WebRTC stream is the viewer. Run these two commands in separate terminals, in **either order** — the viewer listens on the port and the device connects once streaming starts, retrying until the viewer is up. WebRTC installs no streaming certificates on the device, so it has none of the ordering constraint that [HTTP](/ark/client-sdk/streaming#start-order-and-streaming-certificates) has. The `--listen` port must match the port in the device's `--signaling-url`.

```bash
# Open the viewer over WebRTC, listening for a direct P2P connection:
aria_streaming_viewer --transport webrtc --listen 9000
# Start streaming on the device (in another terminal), dialing this host:
aria_gen2 streaming webrtc start --signaling-url tcp://<this-host-ip>:9000 \
    --profile low_latency_streaming --interface wifi_sta
```

See [WebRTC Streaming](/ark/client-sdk/webrtc) for the cross-network signaling setup, the separate signaling-server wheel, and the full flag list.

:::caution WebRTC timestamps: RTP video uses host receive time
On WebRTC, `capture_timestamp_ns` for the **RTP video streams (RGB, SLAM, ET)** is the **host receive time** (RTP carries no device capture timestamp), expressed in the device clock via an offset tracked from the data-channel streams. Everything that arrives on the data channel — IMU, VIO, eye gaze, hand pose, magnetometer, barometer, GPS, audio, and cropped POV images — keeps its **device-clock** timestamps. HTTP timestamps are unchanged.
:::

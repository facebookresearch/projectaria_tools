...
:::warning Important
Always start streaming with the correct certificate name or install certificates **before** starting the streaming server. Starting streaming without specifying the cert name will default to installing new certificates, which would invalidate any old certificates already copied to the server.
:::
...
3. **Certificate matching**: The publisher certificate on the device must match the subscriber certificates on the server (issued from the same Nebula CA)

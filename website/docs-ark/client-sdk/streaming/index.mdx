...
Every method on this page — USB, WiFi router and on-device hotspot — streams over the **HTTP transport**, which is what `aria_streaming_viewer` uses by default (`--transport http`). The alternative transport has its own page: see [WebRTC Streaming](/ark/client-sdk/webrtc) for low-latency peer-to-peer and cross-network streaming.

## Prerequisites

- [Client SDK installed](/ark/client-sdk/start) and virtual environment activated
- Device [authenticated](/ark/client-sdk/authentication) with your PC
...
* **[mp_streaming_demo](/technical-specs/device/profile)**: Default streaming profile for visualization - Optimized for real-time visualization with VIO, eye gaze, and hand pose data
* **[profile9](/technical-specs/device/profile)**: General purpose streaming profile - Balanced configuration for streaming with lower bandwidth requirements
...
For instructions on creating custom profiles, see the [Custom Profile Guide](/technical-specs/device/profile#custom_profile_example).
...
|--------|-----------|----------|------|------|
| **[USB Streaming](#usb-streaming)** | HTTP | Extended sessions, highest quality | Most reliable, no thermal issues | Requires cable connection |
| **[WiFi Router Streaming](#wifi-router-streaming)** | HTTP | Untethered development, mobility | Wireless freedom, good for demos | Thermal concerns, requires network |
| **[On-Device Hotspot Streaming](#on-device-hotspot-streaming)** | HTTP | Field work, no network available | No external network needed | Highest thermal impact, limited duration |
| **[Remote Server Streaming](/ark/client-sdk/python-sdk/streaming-example#http-streaming-to-remote-server)** | HTTP | Dedicated processing server, distributed systems | Process on powerful remote machine | Requires network setup, TLS certificates |
| **[WebRTC Streaming](/ark/client-sdk/webrtc)** | WebRTC | Low-latency P2P; cross-network via signaling | Direct P2P on same network; NAT traversal across networks; [can record to VRS while streaming](/ark/client-sdk/webrtc#recording-while-streaming) | Image timestamps are host receive time |
...
Give `streaming start` a certificate that is already installed, so it does not mint a new one. Install one once with [`aria_gen2 streaming install-certs`](/ark/client-sdk/cli-reference#aria_gen2-streaming-install-certs), note the cert name it prints, and pass that name on every subsequent start:
...
:::tip WebRTC installs no streaming certificates
[WebRTC](/ark/client-sdk/webrtc) does not install streaming certificates on the device, so `aria_gen2 streaming webrtc start` and `aria_streaming_viewer --transport webrtc` can always be started in either order. The one certificate it can involve is the signaling server's own, when the signaling channel runs over TLS — that is a trust anchor you point each peer at, not something installed per device, and it does not affect the start order. See [Trusting a TLS signaling server](/ark/client-sdk/webrtc#trusting-a-tls-signaling-server).
:::
...
Run `aria_gen2 streaming start` first and `aria_streaming_viewer` second, in two terminals. Pinning an already-installed certificate with `--streaming-cert-name` lets you run them in either order — see [Start Order and Streaming Certificates](#start-order-and-streaming-certificates) for more details.
:::
...
* Device authenticated (see [Device Authentication](/ark/client-sdk/authentication) guide)
...
**Default Profile:**
The [`mp_streaming_demo`](/technical-specs/device/profile) profile is used by default, which provides smooth visualization with VIO, eye gaze, and hand pose data. See the [Profiles Technical Specification](/technical-specs/device/profile) for detailed sensor configurations and frame rates for all available profiles.
...
By default the viewer generates its layout from the device calibration. A [Rerun blueprint](https://rerun.io/docs/concepts/blueprint) — a `.rbl` file — replaces that layout with a saved arrangement of views, panels, and visibility settings, so you can keep a workspace you like across sessions.
...
:::caution Blueprints are tied to a Rerun version
The `.rbl` format tracks the Rerun release, which is why the shipped file carries `rerun_0.33` in its name — the Rerun a `rerun-sdk ~=0.33.0` install [brings in](/ark/client-sdk/start#step-3-install-the-client-sdk-python-package). Rerun migrates older blueprints forward when it opens them, but a blueprint saved by a *newer* Rerun than the one you are running may load with missing or misplaced views. If a blueprint does not look right, re-save it from the Rerun version you are running, or drop `--blueprint` to fall back to the auto-generated layout.
:::

#### Stopping the Viewer

This applies to both the HTTP and WebRTC transports. Press <kbd>CTRL</kbd>+<kbd>C</kbd>, or simply **close the Rerun window** — the viewer shuts the receiver down and exits on its own, so it does not keep receiving and decoding into a window that is no longer there. Pass `--no-exit-on-viewer-close` to keep the session running after the window is closed.
...
* Switch transport with [WebRTC Streaming](/ark/client-sdk/webrtc) for low-latency P2P and cross-network sessions

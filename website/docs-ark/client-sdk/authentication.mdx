...
2. Verify the device is discoverable via mDNS:
   - On macOS: `dns-sd -B _recordingmanager._tcp`
   - On Linux: `avahi-browse -r _recordingmanager._tcp` (install with `sudo apt install avahi-utils` if not available)

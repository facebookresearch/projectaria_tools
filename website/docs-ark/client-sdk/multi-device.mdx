...
`aria_multi_device_streaming_viewer` is a live Rerun visualization tool that receives RGB streams from every connected device and renders them side-by-side with TDM-corrected timestamps. The Rerun timeline scrubber on `device_time` produces synchronized cross-device replay.

It is installed as a console script by the Client SDK, so it is on `$PATH` in your activated SDK virtual environment — no sample export needed.
...
| When to use | Tool |
|---|---|
| Live cross-device visualization with synchronized timeline scrubbing | `aria_multi_device_streaming_viewer` (this page) |
...
- [HTTP Streaming Control](/ark/client-sdk/streaming) — single-device streaming

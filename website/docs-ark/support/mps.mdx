...
| 118 | Multi-Recording processing error. The recordings in the group were made on mixed device generations. All recordings in a multi-SLAM group must come from the same device generation. |
| 119 | Submit failed because you requested to bypass the VRS Health Check failure but do not have the required permission to do so. |

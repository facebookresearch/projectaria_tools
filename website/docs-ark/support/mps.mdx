...
| 118 | Multi-Recording processing error. The recordings in the group were not all made on the same Aria device generation. Multi-SLAM requires every recording in a group to come from the same generation. |
| 119 | You asked to bypass a VRS Health Check failure, but your account is not permitted to do so. Process the recording without that option, or reach out to support. |
| 120 | The downloaded MPS result failed validation before it could be unzipped. The server returned an empty, placeholder, or truncated result for a request it had previously reported as succeeded (for example a 1-byte file). Resubmit the request with the `--force` flag to reprocess the recording from scratch and regenerate a fresh result. |
| 121 | The eye gaze calibration recording supplied with `--calib` could not be hashed, encrypted, or uploaded. This aborts the combined request, so every pending feature reports error 121. Use `--force` to retry. |
| 122 | An Aria Gen 2 recording requested for Eye Gaze has no eye tracking camera stream. Other eligible MPS services requested in the same run are still submitted. |

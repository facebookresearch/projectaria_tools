...
* All [MPS output](/technical-specs/mps/data_formats) (trajectories, semi-dense point clouds etc.) continue to be stored in secure cloud storage, so that users can re-download the data at any time. MPS output is not available to Meta researchers or Meta’s affiliates.

...
1. Raw Aria sequences (VRS files) are uploaded to secure cloud storage via MPS CLI.
...
<img width="100%" src={useBaseUrl('/img/docs-ark/mps/mps_data_lifecycle.png')} alt="Diagram of MPS Processing lifecycle, as described above"/>
...
* All [MPS output](/technical-specs/mps/data_formats/slam/data_formats) (trajectories, semi-dense point clouds etc.) continue to be stored in secure cloud storage, for up to 30 days, so that users can re-download the data within the 30 days time window. MPS output is not available to Meta researchers or Meta’s affiliates.

...
`projectaria-mps` is pure Python; the glibc floor comes from `projectaria-tools` and `projectaria-vrs-health-check`, both `manylinux_2_17`. Distributions as old as Ubuntu 20.04 LTS should therefore install, but only 22.04 and 24.04 are tested — older releases are untested rather than known-good.
:::
...
![Unmarked eye gaze calibration recording warning](/img/docs-ark/mps/mps_calibration_unmarked.png)
...
![Calibration not uploaded because eye gaze was not requested](/img/docs-ark/mps/mps_calibration_not_uploaded.png)

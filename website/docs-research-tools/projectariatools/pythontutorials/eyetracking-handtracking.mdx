...
### Basic Eye-tracking Data Access

Eye-tracking data provides gaze direction information from the Aria glasses.

```python
# Query EyeGaze stream
eyegaze_stream_id = vrs_data_provider.get_stream_id_from_label("eyegaze")

if eyegaze_stream_id is None:
    print("This VRS file does not contain on-device eye-tracking data.")
else:
    print(f"Found eye-tracking stream: {eyegaze_stream_id}")

    # Get total number of eye-tracking samples
    ...
    # Access a few eye-tracking data samples
    print("\nFirst few eye-tracking samples:")
    for i in range(min(5, num_eyegaze_samples)):
        eyegaze_data = vrs_data_provider.get_eyegaze_data_by_index(eyegaze_stream_id, i)

        print(f"Sample {i}:")
        print(f"\tTimestamp: {eyegaze_data.tracking_timestamp}")
        print(f"\tConfidence: {eyegaze_data.session_uid}")
        print(f"\tYaw: {eyegaze_data.yaw} rad")
        print(f"\tPitch: {eyegaze_data.pitch} rad")
```

### Eye-tracking Data Structure

Eye-tracking data contains the following key information:

- **Tracking Timestamp**: When the eye-tracking measurement was taken
- **Yaw and Pitch**: Gaze direction in spherical coordinates
- **Session UID**: Unique identifier for the eye-tracking session

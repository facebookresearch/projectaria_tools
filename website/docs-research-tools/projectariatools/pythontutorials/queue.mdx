...
# Tutorial 3: Access Multi-Sensor Data Sequentially
...
This tutorial demonstrated how to efficiently access multi-sensor data from Aria VRS files using the queued API:

- **Sequential Access**: Use `deliver_queued_sensor_data()` to iterate through all sensor data in timestamp order
- **Unified Interface**: Handle different sensor types consistently using the `SensorData` interface
- **Customization**: Control data access with stream selection, time windowing, and subsampling options
- **Processing Efficiency**: Focus on specific sensors, time ranges, or reduce data rates as needed

The queued API is particularly powerful for:
- **Multi-sensor fusion** algorithms that need synchronized data
- **Streaming processing** of large VRS files
- **Selective analysis** of specific sensors or time periods
- **Memory-efficient** processing by controlling data rates

Next, you can explore sensor-specific tutorials to learn about working with specific data types like eye tracking, VIO, or MPS data.

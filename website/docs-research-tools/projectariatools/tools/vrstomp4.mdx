...
The `vrs_to_mp4` tool converts Aria VRS files to standard MP4 video format. This tool extracts camera streams and audio data from VRS files and creates MP4 videos that can be played by standard video players.
...
Therefore we **strongly recommend [the `VrsDataProvider` APIs](https://github.com/facebookresearch/projectaria_tools/blob/2.3.0/examples/Gen2/python_notebooks/Tutorial_1_vrs_data_provider_basics.ipynb) in `projectaria-tools`** to fetch images for CV and AI applications.
:::
...
| `--log_folder` | string | No | Folder to store logs: `mp4_to_vrs_time_ns.csv`, `vrs_to_mp4_log.json`, audio files |

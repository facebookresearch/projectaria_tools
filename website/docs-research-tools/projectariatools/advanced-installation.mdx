...
- [Build `projectaria_tools` python package with type hinting from source code](#python_from_source_type_hint)
...
```bash
cd ${SRC_DIR}
cmake -B ./build/ -S ./ -DPROJECTARIA_TOOLS_BUILD_TOOLS=ON
cd build/
make -j8
```

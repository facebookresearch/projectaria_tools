...
The Aria Research Kit's Semi Dense Point Cloud MPS can be requested as an addition to Trajectory MPS.
...
Users will either need to threshold the pointcloud by setting a maximum allowed inverse distance / distance certainty or correctly weight points by their certainty when using them in downstream tasks.

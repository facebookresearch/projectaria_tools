...
# Project Aria Client SDK and CLI Troubleshooting and Known Issues

## Overview
This page provides troubleshooting information specific to using Project Aria Client SDK.

Go to the [Aria Glasses User Manual](https://facebookresearch.github.io/projectaria_tools/docs/ARK/glasses_manual/glasses_user_manual) for general information about using your glasses.

Go to the [Aria Research Kit (ARK) Troubleshooting and Known Issues page](https://facebookresearch.github.io/projectaria_tools/docs/ARK/troubleshooting/troubleshooting_issues) for general Aria Glasses troubleshooting information.
...
On Linux, this may be due to USB driver issues. See [Fix USB Driver Issues in Linux](https://facebookresearch.github.io/projectaria_tools/docs/ARK/troubleshooting/linux_usb_driver)

### Mobile Companion app doesn't receive authorization code
1. Restart the Mobile Companion app and send `aria auth pair` again with the app in the foreground
2. Revoke any existing certificates (via [Device Settings](/docs/ARK/mobile_companion_app#aria-device-settings) and try again
...
If streaming on Linux does not show any data, try the following steps to address connection issues:
  1. Make sure that you have run the nmcli command shown in [the setup instructions](/docs/ARK/sdk/setup.mdx)

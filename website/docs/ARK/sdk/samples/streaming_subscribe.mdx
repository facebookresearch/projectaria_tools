...
# How to Subscribe to and Display Aria Streaming Data

This page shows how to use the code sample `streaming_subscribe` to subscribe to an Aria glasses datastream and provides a code walkthrough of `streaming_subscribe`'s capabilities.
...
## Stream and subscribe example
In this example you will:
* Start streaming using the CLI
* Subscribe to the live stream using the SDK
* Show a live preview of the sensor data from Aria's RGB and SLAM cameras
...
:::info
To stream data over wifi, use `--interface wifi` instead.
To start streaming wirelessly, use `--ip-address <glasses IP>`.
:::
...
Ensure that Aria CLI is started using the `--use-ephemeral-certs` flag to use ephemeral certificates for streaming data securely.

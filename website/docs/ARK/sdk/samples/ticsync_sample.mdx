...
* `ticsync_recording.py` - create TICSync time synchronized recordings with the option to set:
    ...
    * The recordings to stop and the glasses return to their normal recording state after a specific period of time
* `ticsync_cleanup.py` - `Return glasses to their normal recording state once recording is complete
...
1. Unplug the glasses (if you wish) and record your activity
    5. It may take ~40 seconds for the ticsync algorithm to reach stabilization between all devices. [The TICSync Jupyter notebook tutorial](/ARK/sdk/ticsync.mdx#tutorial) includes how to assess how long it takes for your specific setup and recording profile.
2. Stop recording:
    6. Press the Capture button on the top right of the Aria glasses and then return glasses to their normal state by plugging them in and using ‘ticsync_cleanup’
    7. Plug the glasses back into your computer and use `ticsync_cleanup`

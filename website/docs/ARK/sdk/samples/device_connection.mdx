...
# How to Connect Aria Glasses and Fetch Info Using the Client SDK

This page shows how to run the code sample `device_connect` and provides a code walkthrough of its capabilities.
...
In terminal, go to the samples directory and run `python device_connect`:
...
Aria Device Info: model Aria, serial <serial_number>, manufacturer Facebook
...
By default, `DeviceClient` will attempt to connect to Aria glasses over USB. To connect to glasses over WiFi configure you can configure the `DeviceClient` by creating a `DeviceClientConfig`, setting `ip_v4_address` and setting the config.

:::info
You can find the Glasses IP address in the Companion App under `WiFi` status.
:::

...
* [Troubleshooting and Error Codes](#troubleshooting)

Go to [Getting Started With the MPS CLI](mps_cli_getting_started.mdx) for a quick tutorial showing basic commands using sample data.
...
Go to [MPS CLI Overview](mps_cli.mdx) for a full breakdown of all the features available.
...
* Check [Error Codes](#error_codes) to see what the error was.
...
:::note
If you retry 24 hours after the recording was uploaded, you'll also need to re-upload the data.
:::
...
If you encounter a permissions error while trying to sign in to the MPS CLI, try:
...
Some users have reported that the MPS CLI UI has display issues in Terminal. Using a different terminal app may help. [Why doesn't Textual look good on macOS](https://textual.textualize.io/FAQ/#why-doesnt-textual-look-good-on-macos) provides further details about this issue.

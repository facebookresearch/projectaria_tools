...
In this section, we introduce the Python/C++ API to access sensor data in Aria VRS files ([projectaria_tools/main/core/data_provider](https://github.com/facebookresearch/projectaria_tools/blob/main/core/data_provider)).
...
<TabItem value="python" label="Python">
```
Stream ids can be mapped from labels by `get_stream_id_from_label`
```python
stream_id = provider.get_stream_id_from_label("camera-slam-left")
```
Inversely, you can retrieve a label from stream id by `get_stream_id_from_label`
```python
...
<TabItem value="cpp" label="C++">
```
Stream ids can be mapped from labels by `getStreamIdFromLabel`
```cpp
auto streamId = provider.getStreamIdFromLabel("camera-slam-left");
```
Inversely, you can retrieve a label from stream id by `getLabelFromStreamId`
```cpp
...
<TabItem value="python" label="Python">
```
We support four [time domains](/docs/data_formats/aria_vrs/timestamps_in_aria_vrs): TimeDomain.RECORD_TIME, TimeDomain.DEVICE_TIME, TimeDomain.HOST_TIME and TimeDomain.TIME_CODE.

We also support search by three different time query options:
...
<TabItem value="cpp" label="C++">
```
We support four [time domains](/docs/data_formats/aria_vrs/timestamps_in_aria_vrs): `TimeDomain::RecordTime`, `TimeDomain::RecordTime`, `TimeDomain::HostTime`, `TimeDomain::TimeCode`.
...
iter_data = iter(seq)
while True:
  print(iter_data.get_time_ns(TimeDomain.DEVICE_TIME))
  try:
    next(iter_data)

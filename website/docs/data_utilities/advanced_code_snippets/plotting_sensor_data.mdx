---
sidebar_position: 10
title: Plotting sensor data (Python)
---
...
```python
from projectaria_tools.core import dataprovider, image
from projectaria_tools.core.stream_id import stream_id

vrsfile = "example.vrs"
provider = dataprovider.create_vrs_data_provider(vrsfile)
```

### Save Images in png
Since we support converting image data to numpy arrays, images can be easily converted to PIL images and saved as png files.
```python
...
The above snippets gets the following images saved in the local folder:
<table>
        <tr>
            <td>SLAM images</td>
            <td><img src="/img//data_utilities/advanced_guide/camera-slam-left.png" alt="camera-slam-left" width="320"></img>
            <img src="/img//data_utilities/advanced_guide/camera-slam-right.png" alt="camera-slam-right" width="320"></img></td>
        </tr>
        <tr>
            <td>Eyetracking image</td>
            <td><img src="/img//data_utilities/advanced_guide/camera-eyetracking.png" alt="camera-et"></img></td>
        </tr>
        <tr>
            <td>RGB image</td>
            <td><img src="/img//data_utilities/advanced_guide/camera-rgb.png" alt="camera-rgb"  width="320"></img></td>
        </tr>
</table>


### Plotting IMU
First, we organize data into 6 lists, each stores one axis of a specific IMU
...
```
Then we plot the data with `matplotlib`
```python
plt.figure()
...
Finally we save the plot to pdf files.
```python
plt.savefig("imu.pdf", format="pdf", bbox_inches="tight")
```

### Magnetometer
Plotting magnitometer is similar. First, we organize data into 3 lists, each stores one axis of magnetometer data
```python
...
### Wi-Fi beacon
Wi-Fi beacon are first grouped by their mac bssid
```
...
```
Here we plot the mac address which has > 15 samples
```python
plt.figure()
...
### Bluetooth beacon
First, similar to Wi-Fi, we group data by `unique_id`.
```python
...
```
Then we plot the data per unique_id
```python
plt.figure()
fig, ax = plt.subplots(1, 1, figsize=(6, 5))

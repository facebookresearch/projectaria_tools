...
-----|gt-metadata.json
```
The gt-metadata.json file contains the high-level sequence information such as the included Aria's serial number, the scene name, etc, which can be
loaded and queried by AriaDigitalTwinDataPathProvider.
...
----SkeletonMetaData.json [optional]  # Metadata of skeleton data. Omitted if a sequence does not have skeleton ground truth

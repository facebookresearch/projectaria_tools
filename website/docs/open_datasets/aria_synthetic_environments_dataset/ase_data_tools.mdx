...
`Viewer_projects_ase` displays an interactive view of an ASE sequence with [Rerun](/docs/data_utilities/visualization/visualization_python#an-introduction-to-rerun). It allows you to see all data in 3D context.

You can call as the following (`frame_id` being optional):
```
viewer_projects_ase --dataset_path ase_data/7  --frame_id 745
```
![ASE Viewer Screenshot](/img/open_datasets/aria_synthetic_environments_dataset/rerun-ase.png)

:::tip
- The timeline enables you to switch before device_time and frame_id to know to which image frame_id a camera pose corresponds to

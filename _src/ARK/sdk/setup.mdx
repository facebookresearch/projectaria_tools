...
### Note: Current issue affecting Linux users
There is currently an issue in projectaria\_client\_sdk version 1.2.1 impacting Linux users. Linux users are advised to downgrade to version 1.1.0 as engineering works on a PyPi wheels update to address the issue. To do so, uninstall the current version (`pip uninstall projectaria_client_sdk`), install the working version (`pip install projectaria_client_sdk==1.1.0`), and re-pair your device (`aria auth pair`).
